...
import com.kingdee.eas.base.codingrule.CodingRule;
...
import com.kingdee.eas.fdc.invite.DirectNegotiationApproval.DirectNegotiationApprovalFactory;
...
import com.kingdee.eas.fdc.invite.client.BaseInviteEditUI;
import com.kingdee.eas.fdc.invite.client.DirectAccepterResultPrintDataProvider;
import com.kingdee.eas.fdc.invite.client.InviteChangeFormEditUI;
 ...
 * output class name
 */
public class DirectNegotiationApprovalEditUI extends AbstractDirectNegotiationApprovalEditUI
{
    private static final Logger logger = CoreUIObject.getLogger(DirectNegotiationApprovalEditUI.class);
    
    /**
     * output class constructor
     */
    public DirectNegotiationApprovalEditUI() throws Exception
    {
        super();
    }
    public void onLoad() throws Exception {
    	...
    	DirectRuleInfo info = new DirectRuleInfo();
    	String number = iCodingRuleManager.getNumber(info, OrgConstants.DEF_CU_ID);
    	...
    	if(this.getOprtState() == OprtState.ADDNEW ) {
			...
			for(int i = 0; i < kdtEntry.getRowCount(); i++) {
				...
				row.getCell("hit").setValue(false);
			}
		}
    ...
    protected void verifyInputForSave() throws Exception {
		...
		if(this.kdtEntry.getRowCount()==0){
			...
			SysUtil.abort();
		}else if(this.kdtEntry.getRowCount() < 2) {
			FDCMsgBox.showWarning(this,"供应商不能少于两家！");
			SysUtil.abort();
		} else {
			for(int i=0; i<kdtEntry.getRowCount(); i++) {
				if(kdtEntry.getCell(i, "supplier").getValue()==null ||kdtEntry.getCell(i, "bidAmount").getValue()==null ||
						kdtEntry.getCell(i, "firstAmount").getValue()==null /*|| kdtEntry.getCell(i, "secondAmount").getValue() == null*/) {
		...
		if(txtScope.getText().length() > 2000) {
			MsgBox.showWarning("承包范围最多填写2000个字");
			SysUtil.abort();
		}
		if(txtSpecial.getText().length() > 2000) {
			MsgBox.showWarning("特别说明最多填写2000个字");
			SysUtil.abort();
		}
	}
...
//		loadOther();
	...
	protected ICoreBase getBizInterface() throws Exception {
		return DirectNegotiationApprovalFactory.getRemoteInstance();
	}
	...
	protected BaseInviteInfo createNewDate() {
		DirectNegotiationApprovalInfo info=new DirectNegotiationApprovalInfo();
...
//		info.setDeployType(DeployTypeEnum.SettleBalances);
	...
	protected BaseInviteInfo createNewData() {
		return createNewDate();
	}
	protected CoreBillEntryBaseInfo createNewEntryDate() {
		return new DirectNegotiationApprovalEntryInfo();
	}
...
//		this.kdtEntry.getColumn("secondAmount").setRequired(true);
...
//		filter.getFilterItems().add(new FilterItemInfo("isPass",Integer.valueOf(IsGradeEnum.TEMP_VALUE)));
	...
	private HashMap getProviderDataMap() {
		...
		map.put("billid", editData.getId().toString());//单据id
		return map;
	}
