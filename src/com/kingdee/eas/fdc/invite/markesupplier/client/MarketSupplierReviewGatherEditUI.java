...
//			}
	...
	public void onLoad() throws Exception {
      ...
      //add by 2014-12-18
		...
		String sql = "select a.fid from t_con_contractbill a left join T_FDC_ContractType b on b.fid = a.fcontracttypeid  left join T_ORG_BaseUnit c on c.fid =a.FOrgUnitID" +
				" where 1=1 ";
		if(editData.getOrgUnit()!=null){
			...
			sql = sql+" and c.flongNumber like '"+orgUnitInfo.getLongNumber()+"%'";
		}
...
//		}
        ...
        prmtstatus.setQueryInfo("com.kingdee.eas.fdc.invite.markesupplier.app.F7ContractBillQuery");
        prmtstatus.setVisible(true);
        prmtstatus.setEditable(false);
        
        prmtstatus.setDisplayFormat("$name$");
        prmtstatus.setEditFormat("$number$");
        prmtstatus.setCommitFormat("$name$");
//        prmtstatus.setEntityViewInfo(view);
	    ...
	    prmtstatus.setSelectorCollection(sic);
       	...
       	status_OVR.setFormat(new BizDataFormat("$name$"));
       	
//       	this.kdtContractEntry.getColumn("contract").setEditor(status_CellEditor);
//      	this.kdtContractEntry.getColumn("contract").setRenderer(status_OVR);
      	this.kdtContractEntry.getColumn("contract").setRequired(true);
      	this.kdtContractEntry.getColumn("contract").getStyleAttributes().setLocked(true);
      	
      	this.kdtContractEntry.getColumn("isHasContract").getStyleAttributes().setHided(true);
...
//		kdtContractEntry.getCell(kdtContractEntry.getRowCount()-1, "isHasContract").setValue(true);
		...
		uiWindow.show();
		SelectContractUI ui = (SelectContractUI)uiWindow.getUIObject();
		
		if(ui.getDate()==null||ui.getDate().length==0)
			return;
		
		Set<String> contBillId = new HashSet<String>();
		for (int j = 0; j < this.kdtContractEntry.getRowCount(); j++)
		{
			IRow row = this.kdtContractEntry.getRow(j);
			contBillId.add(UIRuleUtil.getString(row.getCell("contract").getValue()));
		}
		
		for (int i = 0; i < ui.getDate().length; i++) 
		{
			ContractBillInfo billInfo = (ContractBillInfo)ui.getDate()[i];
			if(ui.getDate()[i]==null||contBillId.contains(billInfo.getName()))
				continue;
			IRow row = this.kdtContractEntry.addRow();
			row.getCell("isHasContract").setValue(Boolean.TRUE);
			row.getCell("contract").setValue(billInfo.getName());
			row.getCell("contractAmount").setValue(billInfo.getAmount());
		}
	}
...
//			kdtContractEntry.getCell(0, "isHasContract").setValue(true);
//		}else{
...
//					f7.setEntityViewInfo(view);
			...
			ovrName.setFormat(new BizDataFormat("$number$-$billName$"));
