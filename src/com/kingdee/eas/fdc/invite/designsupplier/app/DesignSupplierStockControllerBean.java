...
public class DesignSupplierStockControllerBean extends AbstractDesignSupplierStockControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.fdc.invite.designsupplier.app.DesignSupplierStockControllerBean");
    /**
     * 批量核准
     */
    protected Map _approve(Context ctx, IObjectPK[] pks)throws BOSException, EASBizException
    {
        ...
        String sql1;
        map = new HashMap();
        DesignSupplierStockInfo supplierStockInfo = null;
        ...
        {
        	supplierStockInfo = getDesignSupplierStockInfo(ctx, pks[i]);
...
//            if(!supplierStockInfo.getState().equals(SupplierStateEnum.SUBMIT) || !supplierStockInfo.getCU().getId().equals(ContextUtil.getCurrentCtrlUnit(ctx).getId()))
        ...
        if(sqlParams.size() != 0){
        	con = null;
        	sql = "update T_DES_DesignSupplierStock set FState = 3 , fauditorid='"+ContextUtil.getCurrentUserInfo(ctx).getId()+"' , fauditDate= getDate() where FID in (" + paramsNum.toString() + ")";
...
//        		DBUtil.executeUpdate(sql1, sqlParams, con);
    ...
    protected Map _unApprove(Context ctx, IObjectPK[] pks)throws BOSException, EASBizException
    {
        ...
        for(int c = pks.length; i < c; i++)
        {
            ...
            boolean flse = false;
            String oql = "select id where supplierNumber.id='"+supplierStockInfo.getId().toString()+"'";
            DesignSupplierEvaluationCollection designEvluColl = DesignSupplierEvaluationFactory.getLocalInstance(ctx).getDesignSupplierEvaluationCollection(oql);
            if(designEvluColl.size()>0){flse = true;}
            if(!supplierStockInfo.getState().equals(SupplierStateEnum.APPROVE)||flse)
            {
            	String exceMsg = "";
            	if(flse){
            		exceMsg = exception.getMessage()+"已经在供应商评定引用！";
            	}else{
            		exceMsg = exception.getMessage();
            	}
                log.addException(exceMsg, notApproveException);
                log.setFailRecordCount(++failCount);
                log.setSuccessRecordCount(--successCount);
                continue;
            }
        ...
        if(sqlParams.size() != 0){
        	con = null;
            sql = "update T_DES_DesignSupplierStock set FState = 2 , fauditorid=null , fauditDate=null where FID in (" + paramsNum.toString() + ")";
            try
            {
                con = getConnection(ctx);
                DBUtil.executeUpdate(sql, sqlParams, con);
	...
	protected IObjectPK _save(Context ctx, IObjectValue model)
		...
		DesignSupplierStockInfo info = (DesignSupplierStockInfo)model;
	...
	protected void _submitTwo(Context ctx, IObjectValue supObjectValue)
			throws BOSException, EASBizException {
		DesignSupplierStockInfo info = (DesignSupplierStockInfo)supObjectValue;
		info.setState(SupplierStateEnum.SUBMIT);
		FDCSQLBuilder builder = new FDCSQLBuilder(ctx);
		String sql = "update T_DES_DesignSupplierStock set FState = 2 where FID = ? ";
	...
	protected Map _importDataBase(Context ctx, IObjectCollection objCollection,
	    ...
	    view.setFilter(info);
	    DesignSupplierStockCollection ssc = DesignSupplierStockFactory.getLocalInstance(ctx).getDesignSupplierStockCollection(view);
		/*
		 * 根据编码判断是否为相同的供应商
		 */
	...
	protected IObjectCollection _getSericeArea(Context ctx)
			throws BOSException, EASBizException {
		DesignServiceAreaCollection ac = new DesignServiceAreaCollection();
		EntityViewInfo view = new EntityViewInfo();
	    FilterInfo info = new FilterInfo();
	    info.getFilterItems().add(new FilterItemInfo("isEnable","1"));
	    view.setFilter(info);
	    ac = DesignServiceAreaFactory.getLocalInstance(ctx).getDesignServiceAreaCollection(view);
		return ac;
	}
...
//	     }
	...
	protected void _delete(Context ctx, IObjectPK pk) throws BOSException, EASBizException {
		DesignSupplierStockInfo info=this.getDesignSupplierStockInfo(ctx, pk);
	...
	protected void _addToSysSupplier(Context ctx, IObjectValue objectValue) throws BOSException, EASBizException {
		DesignSupplierStockInfo info = (DesignSupplierStockInfo)objectValue;
		if(info==null){
			return;
		}else{
			...
			info=DesignSupplierStockFactory.getLocalInstance(ctx).getDesignSupplierStockInfo(new ObjectUuidPK(info.getId()),supplier);
		}
    	...
    	if(supplier == null||supplier.getId()==null){
    		...
    		if(col.size()>0){
        		...
        		filter = new FilterInfo();
        		view.setFilter(filter);
        		filter.getFilterItems().add(new FilterItemInfo("name", "规划设计",CompareType.LIKE));
        		filter.getFilterItems().add(new FilterItemInfo("CU.id", OrgConstants.DEF_CU_ID));
	...
	private Set getSupplierMgeCu(Context ctx,String file) throws BOSException {
		Set set = new HashSet();
		String sql =" select c.fid longnumber from T_BD_OperationPhases a left join T_FDC_ProjectBase b on b.fid=a.FProjectBaseID " +
		"left join t_org_ctrlunit c on c.fid=b.FEngineeProjectID where a.fid='"+file+"'";
		String CuId = null;
		IRowSet rowSet = DbUtil.executeQuery(ctx,sql);
		try {
			while(rowSet.next()){
				if(rowSet.getString("longnumber")!=null)
				{
					CuId = rowSet.getString("longnumber");break;
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	private void updateSysSupplier(Context ctx, DesignSupplierStockInfo info, SupplierInfo supplier) throws BOSException, EASBizException {
		...
		Set cuIds = getSupplierMgeCu(ctx,ctx.get("proId").toString());
	...
	private void setSysSupplierValue(Context ctx, DesignSupplierStockInfo info, SupplierInfo supplier) throws BOSException, EASBizException {
		String sql =" select c.fid longnumber from T_BD_OperationPhases a left join T_FDC_ProjectBase b on b.fid=a.FProjectBaseID " +
				"left join t_org_ctrlunit c on c.fid=b.FEngineeProjectID where a.fid='"+ctx.get("proId")+"'";
		String CuId = null;
		IRowSet rowSet = DbUtil.executeQuery(ctx,sql);
		try {
			while(rowSet.next()){
		...
		supplier.setNumber("SJ-" + info.getNumber());
		...
		CtrlUnitInfo cu = new CtrlUnitInfo();
		cu.setId((CuId!=null)?BOSUuid.read(CuId):BOSUuid.read(OrgConstants.DEF_CU_ID));
		supplier.setCU(cu);
	...
	protected void _beachUpdateHisCount(Context ctx, IObjectPK[] pks,
			boolean isAudit) throws BOSException, EASBizException {
		
	}
	protected void _updateHisCount(Context ctx, IObjectPK pk, boolean isAudit)
			throws BOSException, EASBizException {
	}
}