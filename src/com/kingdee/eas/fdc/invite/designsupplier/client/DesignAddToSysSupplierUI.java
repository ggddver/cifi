 ...
 * output class name
 */
public class DesignAddToSysSupplierUI extends AbstractDesignAddToSysSupplierUI
{
    private static final Logger logger = CoreUIObject.getLogger(DesignAddToSysSupplierUI.class);
    
    /**
     * output class constructor
     */
    public DesignAddToSysSupplierUI() throws Exception
    {
        super();
    }
...
//		super.onLoad();
		...
		DesignSupplierStockCollection col=DesignSupplierStockFactory.getRemoteInstance().getDesignSupplierStockCollection(view);
		for(int i=0;i<col.size();i++){
			DesignSupplierStockInfo info=col.get(i);
    ...
    protected void refresh() throws BOSException{
    	...
    	col=DesignSupplierStockFactory.getRemoteInstance().getDesignSupplierStockCollection(view);
...
//	    	}
    ...
    protected void addSupplierRow(IRow row,DesignSupplierStockInfo info){
	...
	protected void tblCNSame_editStopped(KDTEditEvent e) throws Exception {
		if(((Boolean)this.tblCNSame.getRow(e.getRowIndex()).getCell("isToSys").getValue()).booleanValue()){
			...
			DesignSupplierStockInfo fdcinfo=((DesignSupplierStockInfo)this.tblCNSame.getRow(e.getRowIndex()).getCell("supplierId").getUserObject());
	...
	protected void ckbAllSelectTwo_stateChanged(javax.swing.event.ChangeEvent e) throws Exception
    {
    	for(int i=0;i<this.tblCNSame.getRowCount();i++){
			...
			if(this.ckbAllSelectTwo.isSelected()){
				...
				DesignSupplierStockInfo fdcinfo=((DesignSupplierStockInfo)this.tblCNSame.getRow(i).getCell("supplierId").getUserObject());
    	        	   ...
    	        	   SelectorItemCollection sel=new SelectorItemCollection();
    	   			   ...
    	   			   for(int i=0;i<tblCNSame.getRowCount();i++){
    	   				   if(((Boolean)tblCNSame.getRow(i).getCell("isToSys").getValue()).booleanValue()){
    	   					...
    	   					DesignSupplierStockInfo fdcinfo=((DesignSupplierStockInfo)tblCNSame.getRow(i).getCell("supplierId").getUserObject());
    	   					DesignSupplierStockFactory.getRemoteInstance().updatePartial(fdcinfo, sel);
