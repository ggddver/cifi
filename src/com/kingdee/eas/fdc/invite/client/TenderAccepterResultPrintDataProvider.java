...
import com.kingdee.eas.fdc.sellhouse.client.SignManagePrintDataProvider;
import com.kingdee.jdbc.rowset.IRowSet;

public class TenderAccepterResultPrintDataProvider extends FDCBillDataProvider{
	private HashMap map = null;
	private static final Logger logger = Logger.getLogger(TenderAccepterResultPrintDataProvider.class);
	
	public TenderAccepterResultPrintDataProvider(String billId, IMetaDataPK mainQuery) {
		super(billId, mainQuery);
		// TODO Auto-generated constructor stub
	}
	
	public TenderAccepterResultPrintDataProvider(String billId,IMetaDataPK mainQuery,HashMap map)
	{
		super(billId, mainQuery);
		this.map = map;
	}
	...
	public IRowSet getOtherSubRowSet(BOSQueryDataSource ds) {
		if (ds.getID().equalsIgnoreCase("RECORD")) {
			IRowSet iRowSet = null;
			try {
				IQueryExecutor exec = QueryExecutorFactory.getRemoteInstance(new MetaDataPK(
						"com.kingdee.eas.fdc.invite.app.SupplierInviteRecordPrintQuery"));
				
				exec.option().isAutoTranslateEnum = true;
				EntityViewInfo ev = new EntityViewInfo();
				FilterInfo filter = new FilterInfo();
				
				filter.getFilterItems().add(new FilterItemInfo("inviteProject.id",map.get(TenderAccepterResultEditUI.INVITEPROJECTID)));
				filter.getFilterItems().add(new FilterItemInfo("supplier.id",map.get(TenderAccepterResultEditUI.SUPPLIERIDSET), CompareType.INCLUDE));
			...
			return iRowSet;
		}else if (ds.getID().equalsIgnoreCase("INVITEPROJECT")) {
			IRowSet iRowSet = null;
			try {
				IQueryExecutor exec = QueryExecutorFactory.getRemoteInstance(new MetaDataPK(
				...
				filter.getFilterItems().add(new FilterItemInfo("id", map.get(TenderAccepterResultEditUI.INVITEPROJECTID).toString()));
