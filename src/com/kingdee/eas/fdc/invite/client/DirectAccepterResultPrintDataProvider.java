...
public class DirectAccepterResultPrintDataProvider extends FDCBillDataProvider{
	private static final Logger logger = Logger.getLogger(DirectAccepterResultPrintDataProvider.class);
	private HashMap map = null;
	
	public DirectAccepterResultPrintDataProvider(String billId, IMetaDataPK mainQuery) {
		super(billId, mainQuery);
		// TODO Auto-generated constructor stub
	}
	
	public DirectAccepterResultPrintDataProvider(String billId,IMetaDataPK mainQuery,HashMap map)
	{
		super(billId, mainQuery);
		this.map = map;
	}
	...
	public IRowSet getOtherSubRowSet(BOSQueryDataSource ds) {
		...
		//获取议标审批中标单位信息
		if (ds.getID().equalsIgnoreCase("Hit")) {
			IRowSet iRowSet = null;
			try {
				IQueryExecutor exec = QueryExecutorFactory.getRemoteInstance(new MetaDataPK(
						"com.kingdee.eas.fdc.invite.DirectNegotiationApproval.app.DirectNegotiationApprovalPrintQuery"));
				
				exec.option().isAutoTranslateEnum = true;
				EntityViewInfo ev = new EntityViewInfo();
				FilterInfo filter = new FilterInfo();
				
				filter.getFilterItems().add(new FilterItemInfo("id", map.get("billid")));
