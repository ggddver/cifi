...
public class SupplierQualifyEditUI extends AbstractSupplierQualifyEditUI
{
    ...
    private InviteDocumentsEditUI inviteDocumentsEditUI = null;
...
//			this.actionALine_actionPerformed(null);
    	...
    	String projectId = this.editData.getInviteProject().getId().toString();
    	String oql = "where InviteProject.id='" + projectId + "' order by createTime";
    	InviteDocumentsCollection inviteDocCollOld = InviteDocumentsFactory.getRemoteInstance().getInviteDocumentsCollection(oql);
    	oql = "where SourceBillId='" + txtNumber.getText() +"'";
    	InviteDocumentsCollection inviteDocCollNew = InviteDocumentsFactory.getRemoteInstance().getInviteDocumentsCollection(oql);
    	if(inviteDocCollOld.size() > 0 && inviteDocCollNew.size() == 0) {
            ...
            uiContext.put("inviteProject", this.editData.getInviteProject());
            uiContext.put("ID", inviteDocCollOld.get(0).getId().toString());
            
            inviteDocumentsEditUI = (InviteDocumentsEditUI) UIFactoryHelper.initUIObject(InviteDocumentsEditUI.class.getName(), uiContext, null, OprtState.VIEW);
            initInviteDocEditUI();
            panel.setViewportView(inviteDocumentsEditUI);
    		panel.setKeyBoardControl(true);
    		panel.setEnabled(false);
    		kDTabbedPane1.add(panel, "招标文件");
    	} else if(inviteDocCollNew.size() > 0) {
    		KDScrollPane panel = new KDScrollPane();
    		panel.setMinimumSize(new Dimension(1013,600));		
    		panel.setPreferredSize(new Dimension(1013,600));
        	UIContext uiContext = new UIContext(this);
            uiContext.put("inviteProject", this.editData.getInviteProject());
            uiContext.put("ID", inviteDocCollNew.get(0).getId().toString());
        	...
        	UIContext uiContext = new UIContext(this);
            uiContext.put("inviteProject", this.editData.getInviteProject());
            uiContext.put("MainMenuName", "招标/议标文件");
            
            inviteDocumentsEditUI = (InviteDocumentsEditUI) UIFactoryHelper.initUIObject(InviteDocumentsEditUI.class.getName(), uiContext, null, OprtState.ADDNEW);
            initInviteDocEditUI();
            panel.setViewportView(inviteDocumentsEditUI);
    		panel.setKeyBoardControl(true);
    		panel.setEnabled(false);
    	...
    	if(getOprtState() == OprtState.EDIT) {
    		inviteDocumentsEditUI.setOprtState(OprtState.EDIT);
    	}
    }
    
    private void initInviteDocEditUI() {
    	inviteDocumentsEditUI.contProject.setVisible(false);
		inviteDocumentsEditUI.contProject.setVisible(false);
		inviteDocumentsEditUI.contName.setVisible(false);
		inviteDocumentsEditUI.btnShowProject.setVisible(false);
		inviteDocumentsEditUI.contCreator.setVisible(false);
		inviteDocumentsEditUI.contAuditor.setVisible(false);
		inviteDocumentsEditUI.contRespDept.setVisible(false);
		inviteDocumentsEditUI.contCreateTime.setVisible(false);
		inviteDocumentsEditUI.contAuditTime.setVisible(false);
    }
    @Override
    public void actionEdit_actionPerformed(ActionEvent e) throws Exception {
    	// TODO Auto-generated method stub
    	super.actionEdit_actionPerformed(e);
    	if(inviteDocumentsEditUI != null) {
    		inviteDocumentsEditUI.actionEdit_actionPerformed(e);
    	}
    }
    @Override
    public void actionSave_actionPerformed(ActionEvent e) throws Exception {
    	...
    	if(inviteDocumentsEditUI != null) {
    		inviteDocumentsEditUI.editData.setSourceBillId(txtNumber.getText());
    		inviteDocumentsEditUI.actionSave_actionPerformed(e);
    	}
    }
    @Override
    ...
    public void actionRemove_actionPerformed(ActionEvent e) throws Exception {
    	...
    	if(inviteDocumentsEditUI != null) {
    		...
    		filter.getFilterItems().add(new FilterItemInfo("id", inviteDocumentsEditUI.editData.getId().toString(), CompareType.EQUALS));
    		InviteDocumentsFactory.getRemoteInstance().delete(filter);
    	}
    }
